  ...
  public:
	...
	inline static FrameBuffer* New(int drm_fd, uint32_t width, uint32_t height)
	{
		...
		try
		{
			...
			buffer->addr = checked::mmap(0, create.size, PROT_READ | PROT_WRITE, MAP_SHARED, drm_fd,
			                             map.offset);
...
class DRM : public LinuxFile
{
  private:
	uint32_t crtc_id;
	struct drm_mode_crtc saved_mode_crtc;
  ...
  public:
	...
	inline FrameBuffer* initialize()
	{
		...
		memset(&saved_mode_crtc, 0, sizeof(saved_mode_crtc));
		saved_mode_crtc.crtc_id = crtc_id;
		ioctl<DRM_IOCTL_MODE_GETCRTC>(&saved_mode_crtc);
	...
	inline void deinitialize()
	{
		...
		ioctl<DRM_IOCTL_MODE_CURSOR>(&arg);
		cursor.destroy(raw());
		ioctl<DRM_IOCTL_MODE_SETCRTC>(&saved_mode_crtc);
	}
  ...
  public:
	...
	inline void OnREL(struct input_event event)
	{
		switch (event.code)
		{
			case REL_X:
			{
				x = std::clamp(x + 2 * event.value, 0, (int)drm.monitor.mode->hdisplay);
			...
			case REL_Y:
			{
				y = std::clamp(y + 2 * event.value, 0, (int)drm.monitor.mode->vdisplay);
				freerdp_input_send_mouse_event(context.input, PTR_FLAGS_MOVE, x, y);
				drm.MoveCursor(x, y);
				break;
			}
...
class Touchpad : public EvDev
{
  public:
	inline Touchpad(const char* path) : EvDev(path)
	{
	}
	virtual std::optional<struct input_event> event()
	{
		...
		if (read<true>(&event, sizeof(event)) == sizeof(event))
		{
			printf("touchpad: type = %d, code = %d, value = %d\n", event.type, event.code,
			       event.value);
		}
		return {};
	}
  ...
  private:
	...
	void process_device(struct udev_device* dev)
	{
		    ...
		    (is_touchpad && strcmp(is_touchpad, "1") == 0))
		{
			...
			{
				devices[devnode] = is_touchpad ? std::make_unique<Touchpad>(devnode)
				                               : std::make_unique<EvDev>(devnode);
			}
...
int main(int argc, char* argv[])
{
	...
	EntryPoints.ClientNew = [](freerdp* instance, rdpContext* context) -> BOOL
	{
		...
		instance->PostConnect = [](freerdp* instance) -> BOOL
		{
			...
			pointer.Set = [](rdpContext* context, rdpPointer* pointer) -> BOOL
			{
				...
				//  Data is appended to pointer
				reinterpret_cast<drmContext*>(context)->drm.cursor.copy((void*)(pointer + 1));
				return TRUE;
			};
			pointer.SetNull = [](rdpContext* context) -> BOOL
			{
				reinterpret_cast<drmContext*>(context)->drm.cursor.set(0);
				return TRUE;
			};
	...
	ScopeGuard Client([&]() { freerdp_client_start(context.get()); },
	                  [&]() { freerdp_client_stop(context.get()); });
	ScopeGuard Instance([&]() { freerdp_connect(context->instance); },
	...
	while (!freerdp_shall_disconnect_context(context.get()))
	{
		...
		freerdp_check_event_handles(context.get());
		sched_yield();
	}
